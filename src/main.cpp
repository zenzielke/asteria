...
#if defined(NDEBUG)
# error "Asteriacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xda49b48d661e530ebe7056b2a649ce42b100848f9d2e6dfdfe5e61732effd3f6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Asteriacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Asteriacoin Signed Message:\n";
...
{
    // Asteriacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Asteriacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Asteriacoin: 2 days
static const int64 nTargetSpacing = 2 * 60; // Asteriacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Asteriacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Asteriacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x157e9a2ed7186c0170328193b7498874d701b32f134e8cf6839e224d23fa7b0e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "YouTube streamer who helped drive surge gamestop shared ID'ed";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dda86c3330d2572e67f9806f60962678befc5afa6e3b2b3477bbf95f9dbb785f39ef3076a19240ac177daee47a80045448687dd335a40bf44ebde102700a2fd0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1611977328;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084627766;

        if (fTestNet)
        {
            block.nTime    = 1611977292;
            block.nNonce   = 386467869;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("01c6c1fa369412a3b51ed5f1d0a9dfe03381ba017a1ad4bad82a9544a44c35ff"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbc, 0xd2 }; // Asteriacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AsteriacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AsteriacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AsteriacoinMiner(CWallet *pwallet)
{
    printf("AsteriacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("asteriacoin-miner");
    ...
    try { loop {
        ...
        printf("Running AsteriacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AsteriacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AsteriacoinMiner, pwallet));
}
